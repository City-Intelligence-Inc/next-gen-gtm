...
async def upload_document(username: str, request: dict = None):
    ...
    # Index into user's PRIVATE ChromaDB collection (isolated from other users)
    chunks_indexed = 0
    try:
        from app.services.rag_service import index_user_document
        chunks_indexed = index_user_document(username, doc_id, title, content)
    except Exception as e:
        logger.error(f"Failed to index user doc into ChromaDB: {e}")

    return {"status": "saved", "doc_id": doc_id, "title": title, "chars": len(content), "chunks_indexed": chunks_indexed}
